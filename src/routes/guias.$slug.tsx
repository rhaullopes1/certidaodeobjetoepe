import { createFileRoute, Link, notFound } from "@tanstack/react-router";
import { PageShell } from "@/components/site/page-shell";
import { AlternativasContato } from "@/components/site/alternativas-contato";
import { GUIAS_SEO, GUIA_POR_SLUG } from "@/lib/guias-seo";
import { adsenseScripts } from "@/lib/adsense";

const SITE = "https://certidaodeobjetoepe.org";

export const Route = createFileRoute("/guias/$slug")({
  loader: ({ params }) => {
    const guia = GUIA_POR_SLUG.get(params.slug);
    if (!guia) throw notFound();
    return {
      guia,
      relacionados: GUIAS_SEO.filter((g) => g.slug !== guia.slug).slice(0, 3),
    };
  },
  head: ({ params, loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Guia não encontrado" }, { name: "robots", content: "noindex" }] };
    }
    const g = loaderData.guia;
    const url = `${SITE}/guias/${params.slug}`;
    return {
      meta: [
        { title: g.titulo },
        { name: "description", content: g.descricao },
        { property: "og:title", content: g.titulo },
        { property: "og:description", content: g.descricao },
        { property: "og:url", content: url },
        { property: "og:type", content: "article" },
        { property: "og:locale", content: "pt_BR" },
        { name: "twitter:card", content: "summary_large_image" },
        { name: "twitter:title", content: g.titulo },
        { name: "twitter:description", content: g.descricao },
      ],
      links: [
        { rel: "canonical", href: url },
        { rel: "alternate", hrefLang: "pt-BR", href: url },
        { rel: "alternate", hrefLang: "x-default", href: url },
      ],
      scripts: [
        ...adsenseScripts,
        {
          type: "application/ld+json",
          children: JSON.stringify({
            "@context": "https://schema.org",
            "@type": "BreadcrumbList",
            itemListElement: [
              { "@type": "ListItem", position: 1, name: "Início", item: SITE },
              { "@type": "ListItem", position: 2, name: "Guias", item: `${SITE}/guias` },
              { "@type": "ListItem", position: 3, name: g.h1, item: url },
            ],
          }),
        },
        {
          type: "application/ld+json",
          children: JSON.stringify({
            "@context": "https://schema.org",
            "@type": "FAQPage",
            mainEntity: g.faq.map((f) => ({
              "@type": "Question",
              name: f.q,
              acceptedAnswer: { "@type": "Answer", text: f.a },
            })),
          }),
        },
      ],
    };
  },
  component: GuiaPage,
});

function GuiaPage() {
  const { guia, relacionados } = Route.useLoaderData();

  return (
    <PageShell>
      <article className="w-full px-4 py-14 sm:px-8 lg:py-20">
        <div className="mx-auto w-full max-w-3xl">
          <nav aria-label="Trilha de navegação" className="text-xs text-muted-foreground">
            <Link to="/" className="hover:text-foreground">Início</Link>
            <span className="px-2">/</span>
            <Link to="/guias" className="hover:text-foreground">Guias</Link>
          </nav>

          <h1 className="mt-4 font-display text-3xl font-bold leading-tight break-words sm:text-4xl">
            {guia.h1}
          </h1>
          <p className="mt-4 text-base leading-relaxed text-muted-foreground">{guia.resumo}</p>

          <Link
            to="/solicitar"
            className="mt-7 inline-flex rounded-full bg-gold px-6 py-3 text-sm font-bold text-accent-foreground"
          >
            Solicitar certidão online
          </Link>
          <AlternativasContato className="mt-4" />

          {guia.secoes.map((s) => (
            <section key={s.h2} className="mt-12">
              <h2 className="font-display text-xl font-bold break-words sm:text-2xl">{s.h2}</h2>
              {s.paragrafos.map((p) => (
                <p key={p} className="mt-4 text-base leading-relaxed text-muted-foreground">
                  {p}
                </p>
              ))}
              {s.lista ? (
                <ul className="mt-4 space-y-2">
                  {s.lista.map((item) => (
                    <li
                      key={item}
                      className="flex gap-3 text-base leading-relaxed text-muted-foreground"
                    >
                      <span aria-hidden className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-gold" />
                      <span className="break-words">{item}</span>
                    </li>
                  ))}
                </ul>
              ) : null}
            </section>
          ))}

          <section className="mt-14">
            <h2 className="font-display text-xl font-bold sm:text-2xl">Perguntas frequentes</h2>
            <dl className="mt-6 space-y-6">
              {guia.faq.map((f) => (
                <div key={f.q} className="rounded-2xl border border-border bg-card p-5">
                  <dt className="font-semibold break-words">{f.q}</dt>
                  <dd className="mt-2 text-sm leading-relaxed text-muted-foreground">{f.a}</dd>
                </div>
              ))}
            </dl>
          </section>

          <section className="mt-14">
            <h2 className="font-display text-xl font-bold sm:text-2xl">Continue lendo</h2>
            <ul className="mt-5 space-y-3">
              {relacionados.map((g) => (
                <li key={g.slug}>
                  <Link
                    to="/guias/$slug"
                    params={{ slug: g.slug }}
                    className="block rounded-xl border border-border bg-card px-5 py-4 text-sm font-semibold break-words transition-colors hover:bg-secondary"
                  >
                    {g.h1}
                  </Link>
                </li>
              ))}
              <li>
                <Link
                  to="/tribunais"
                  className="block rounded-xl border border-border bg-card px-5 py-4 text-sm font-semibold transition-colors hover:bg-secondary"
                >
                  Certidão por tribunal (TJ, TRF e TRT)
                </Link>
              </li>
              <li>
                <Link
                  to="/certidao-de-objeto-e-pe"
                  className="block rounded-xl border border-border bg-card px-5 py-4 text-sm font-semibold transition-colors hover:bg-secondary"
                >
                  Certidão de Objeto e Pé por estado
                </Link>
              </li>
            </ul>
          </section>
        </div>
      </article>
    </PageShell>
  );
}
